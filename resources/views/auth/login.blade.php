...
<head>
    ...
    <title>Masuk ke Akun — TaskFlow</title>
    <!-- Tailwind CSS CDN -->
...
<body class="relative min-h-screen bg-bintik text-slate-800 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 selection:bg-indigo-500 selection:text-white">
    ...
    <div class="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div class="absolute -top-32 -left-32 w-96 h-96 bg-indigo-200/40 rounded-full blur-[100px]"></div>
        <div class="absolute top-1/3 -right-32 w-[28rem] h-[28rem] bg-purple-200/35 rounded-full blur-[110px]"></div>
        <div class="absolute -bottom-32 left-1/3 w-96 h-96 bg-sky-200/40 rounded-full blur-[100px]"></div>
    </div>
    ...
    <div class="relative z-10 sm:mx-auto sm:w-full sm:max-w-md">
        <div class="flex flex-col items-center justify-center">
            ...
            <p class="mt-2 text-center text-xs sm:text-sm text-slate-600">
                Belum memiliki akun?
                <a href="{{ route('register') }}" class="font-bold text-indigo-600 hover:text-indigo-700 underline underline-offset-4 decoration-indigo-300 hover:decoration-indigo-600 transition">
    ...
    <div class="relative z-10 mt-7 sm:mx-auto sm:w-full sm:max-w-md">
        <div class="bg-white/95 backdrop-blur-xl py-8 px-6 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.08)] rounded-3xl sm:px-10 border border-slate-200/90 relative overflow-hidden">
            ...
            <div class="absolute top-0 inset-x-0 h-1.5 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-500"></div>

            <!-- Pesan Notifikasi Sukses -->
            @if (session('success'))
                <div class="mb-5 bg-emerald-50 border-l-4 border-emerald-500 text-emerald-800 p-3.5 rounded-r-xl text-xs flex items-center space-x-2.5 shadow-xs">
            ...
            <form action="{{ route('login.submit') }}" method="POST" class="space-y-4">
                ...
                <div class="flex items-center justify-between py-1">
                    <label class="flex items-center space-x-2.5 cursor-pointer select-none">
                        <input type="checkbox" name="remember" class="w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500 transition">
